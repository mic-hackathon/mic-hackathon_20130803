...
class NBVH16 : public BVHBase {
 private:
  ...
  struct SIMDBVHNode{
    ...
    Children children[16]; //4 children
  ...
  void collapse2NBVH(SIMDBVHNode* node) {
    float bboxes[2][3][16] __attribute__ ((aligned (64))) = {0};
    SIMDBVHNode newnode;

    newnode.axis_top = node->axis_top;
    newnode.axis_left = node->axis_left;
    newnode.axis_right = node->axis_right;

    for (int i = 0; i < 4; ++i) {
      if (node->children[i].u.node_.flag_ == 0) {
	// ノード
	// は展開	
	SIMDBVHNode *c = simd_nodes_[node->children[i].u.node_.index_];

	float bbox[2][3][16] __attribute__ ((aligned (64))) = {0};
	for(int m = 0; m < 2; m++){//minmax
	  for(int j = 0; j < 3; j++){//xyz
	    _mm512_store_ps(bbox[m][j], c->bboxes[m][j]);
	  }
	}

	for (int a = 0; a < 3; ++a)
	  for (int b = 0; b < 4; ++b) {
	    bboxes[0][a][4 * i + b] = bbox[0][a][b];
	    bboxes[1][a][4 * i + b] = bbox[1][a][b];
	  }
	for (int a = 0; a < 4; ++a) {
	  newnode.children[4 * i + a] = c->children[a];
	  if (c->children[a].u.node_.flag_ == 0) {
	    collapse2NBVH(simd_nodes_[c->children[a].u.node_.index_]);
	  }
	}
	...
	// は何もしない
	for (int a = 0; a < 4; ++a) {
	  newnode.children[4 * i + a] = node->children[i];
	}
      }
    }
    ...
    for(int m = 0; m < 2; m++){//minmax
      for(int j = 0; j < 3; j++){//xyz
	newnode.bboxes[m][j] = _mm512_load_ps(bboxes[m][j]);
      }
    }
 ...
 public:
  ...
  bool intersect(const Ray &ray, Hitpoint* hitpoint) {
	...
	if (item.u.node_.index_ >= simd_nodes_.size()) {
	  std::cout << "HOGE";
	  return false;
	}
	...
	if (HitMask) {
	  ...
	  const int neoHitMask = ((HitMask & 0x8000) >> 12) + ((HitMask & 0x800) >> 9) + ((HitMask & 0x80) >> 6) + ((HitMask & 0x8) >> 3);
	  ...
	  for (int i = 0; i < 4; ++i) {
	    ...
	    const int idx = bboxOrder & 0x3;
	    for (int j = 0; j < 4; ++j) {
	      ++todoNode;
	      nodeStack[todoNode] = node.children[4 * idx + j];
	    }
