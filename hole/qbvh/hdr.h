...
HDRImage(const int width, const int height) :
  ...
  void load_unsafe(const std::string &filename) {
	...
	image_[(height_ - 1 - y) * width_ + x].x_ = tmp_data[(y * width_ + x) * 4 + 0] * pow(2.0f, e - 128.0f) / 256.0f;
	image_[(height_ - 1 - y) * width_ + x].y_ = tmp_data[(y * width_ + x) * 4 + 1] * pow(2.0f, e - 128.0f) / 256.0f;
	image_[(height_ - 1 - y) * width_ + x].z_ = tmp_data[(y * width_ + x) * 4 + 2] * pow(2.0f, e - 128.0f) / 256.0f;
